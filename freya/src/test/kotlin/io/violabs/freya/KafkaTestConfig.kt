...
open class KafkaTestConfig {
    ...
    open fun userConsumerTemplate(): ReactiveKafkaConsumerTemplate<String, UserMessage> {
        return userConsumerConfigs()
            .let { ReceiverOptions.create<String, UserMessage>(it).subscription(listOf("user")) }
            .let { ReactiveKafkaConsumerTemplate(it) }
    }

    @Bean
    open fun userKafkaConsumer(consumerTemplate: ReactiveKafkaConsumerTemplate<String, UserMessage>): UserKafkaConsumer {
        return UserKafkaConsumer(consumerTemplate)
    }

    @Bean
    open fun userKafkaListenerContainerFactory(producerConfigs: Map<String, Any>): ConcurrentKafkaListenerContainerFactory<String, UserMessage> {
    ...
    class UserKafkaConsumer(private val consumerTemplate: ReactiveKafkaConsumerTemplate<String, UserMessage>) {
